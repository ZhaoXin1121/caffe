...
template <typename Dtype> 
vector<vector<double> > Net<Dtype>::Get_feature(const vector<string>& layers_name) {
  int n=layers_.size();
  int m=layers_name.size();
  vector<double> layer_feature;	
  vector<vector<double> >feature;
  for(int i=1;i<n;i++){
	for(int j=0;j<m;j++){
		if(layer_names_[i].compare(layers_name[j])==0){
				   for(int u = 0;u < top_vecs_[i][0]->num();u++){
                                	for(int c = 0;c < top_vecs_[i][0]->channels();c++){
                                        	for(int h = 0;h < top_vecs_[i][0]->height();h++){
                                                	for(int w = 0;w < top_vecs_[i][0]->width();w++){
								layer_feature.push_back(top_vecs_[i][0]->data_at(u,c,h,w));
							}
						}
					}
				 }
				feature.push_back(layer_feature);
				layer_feature.clear();
		}
	 }
 }
